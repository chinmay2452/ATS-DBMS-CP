...
-- 2. "Display all applications with applicant details and current stage"
SELECT 
    ap.application_id,
    a.full_name,
...
-- 4. "Get list of hired applicants with job details"
SELECT 
    a.full_name,
    j.job_title,
    ap.application_status
...
-- 7. "Retrieve all applications for active jobs"
SELECT 
    a.full_name,
    j.job_title,
    ap.application_status
...
-- 3. "Calculate average salary offered"
SELECT 
...
-- 5. "Calculate number of applications per stage"
SELECT 
    rs.stage_name,
...
-- 6. "Count number of hired applicants per job"
SELECT 
    j.job_title,
...
-- 7. "Find highest salary offered"
SELECT 
    MAX(salary_package) AS highest_salary
FROM offer;

-- 8. "Identify applicants at interview stage but not yet hired"
SELECT 
    a.full_name,
    rs.stage_name AS current_stage
FROM application ap
...
-- 9. "Identify jobs with no applications"
SELECT 
    j.job_title
FROM job j
LEFT JOIN application ap ON j.job_id = ap.job_id
...
-- 10. "Calculate conversion rate (Hired vs Total Applications)"
SELECT 
